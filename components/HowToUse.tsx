...
export default function HowToUse() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Image reveal
      gsap.from(".how-image", {
        scale: 1.06, opacity: 0, duration: 1.1, ease: "power3.out",
        scrollTrigger: { trigger: ".how-image", start: "top 85%" },
      });

      // Desktop: draw the connecting lines top → bottom, then reveal each step
      const stepEls = gsap.utils.toArray<Element>(".how-step");
      const lineEls = gsap.utils.toArray<Element>(".step-connector");

      stepEls.forEach((step, i) => {
        const tl = gsap.timeline({
          scrollTrigger: { trigger: step, start: "top 88%" },
        });
        tl.from(step, { y: 36, opacity: 0, duration: 0.7, ease: "power3.out" });
        if (lineEls[i]) {
          tl.from(lineEls[i], { scaleY: 0, transformOrigin: "top", duration: 0.5, ease: "power2.out" }, "-=0.3");
        }
      });

      // Footnote
      gsap.from(".how-footnote", {
        y: 20, opacity: 0, duration: 0.7, ease: "power2.out",
        scrollTrigger: { trigger: ".how-footnote", start: "top 90%" },
      });
    }, sectionRef);
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-6">
        ...
        <div className="md:hidden">
          <div className="how-header">
            ...
            <h2 className="text-[#1a0a3d] font-bold mb-4" style={{ fontFamily: "var(--font-playfair)", fontSize: "24px" }}>
          ...
          <div className="how-footnote mt-5 pt-4 border-t border-[#ede9fe]">
        ...
        <div className="hidden md:grid md:grid-cols-2 gap-16 items-center">
          ...
          <div>
              ...
              {steps.map((step, i) => (
                <div key={step.num} className="how-step flex gap-6 items-start">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    {i < steps.length - 1 && <div className="step-connector w-px h-8 bg-[#ede9fe]" />}
                  </div>
                  <div className="pt-2">
            ...
            <p className="how-footnote text-[#888] text-xs leading-relaxed mt-8 pt-6 border-t border-[#ede9fe]" style={{ fontFamily: "var(--font-inter)" }}>{footnote}</p>
          </div>
        </div>
